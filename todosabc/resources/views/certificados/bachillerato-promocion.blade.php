...
echo "<p style='text-align: justify;margin-top: 0;'>Obtuvo las siguientes calificaciones durante el PRIMER CURSO DE BACHILLERATO GENERAL UNIFICADO.</p>";
echo "<center>";
...
$total_materias = 1;
if(isset($calificaciones_fase_1)){	
	if(count($calificaciones_fase_1)>0){
		$total_materias = count($calificaciones_fase_1);
	}
	$promedio_final = 0;
	foreach ($calificaciones_fase_1 as $i => $cal) {
...
if(isset($calificaciones_fase_1) and count($calificaciones_fase_1)>0){
	echo "<td colspan='2' style='border: black 1px solid;text-align:center;'>".recuperarComportamiento($calificaciones_fase_1[$total_materias-1]->comportamiento)."</td>";
}else{
...
echo "<p style='text-align: justify;'>Por lo tanto, es promovido/a al: SEGUNDO CURSO DE BACHILLERATO GENERAL UNIFICADO.";
...
echo "de Educación Intercultural y demás normas vigentes, certifica que el/la estudiante:</p><br>";
echo "<p style='margin-top: 0;margin-bottom: 0;text-align: center;font-size: 18px; font-weight: bold; font-style: italic;'>$matriculado->nombres_aspirantes</p><br>";
echo "<p style='text-align: justify;margin-top: 0;'>Obtuvo las siguientes calificaciones durante el SEGUNDO CURSO DE BACHILLERATO GENERAL UNIFICADO.</p>";
echo "<center>";
...
//iterar calificaciones
$total_materias = 1;
if(isset($calificaciones_fase_2)){	
	if(count($calificaciones_fase_2)>0){
		$total_materias = count($calificaciones_fase_2);
	}
	$promedio_final = 0;
	foreach ($calificaciones_fase_2 as $i => $cal) {
		$promedio_final += $cal->nota_final;
		echo "<tr>";
		echo "<td style='border: black 1px solid;'>".strtoupper($cal->asignatura)."</td>";		
		echo "<td style='border: black 1px solid;text-align:center;'>".escribirNota($cal->nota_final)."</td>";
...
echo "<tr style='background-color: #e6e6e6;'>";
echo "<td style='border: black 1px solid;font-weight: bold;'>EVALUACIÓN DEL COMPORTAMIENTO</td>";
if(isset($calificaciones_fase_2) and count($calificaciones_fase_2)>0){
	echo "<td colspan='2' style='border: black 1px solid;text-align:center;'>".recuperarComportamiento($calificaciones_fase_2[$total_materias-1]->comportamiento)."</td>";
}else{
	echo "<td colspan='2' style='border: black 1px solid;text-align:center;'></td>";
}
echo "</tr>";
...
echo "<p style='text-align: justify;'>Por lo tanto, es promovido/a al: TERCER CURSO DE BACHILLERATO GENERAL UNIFICADO.";
echo " Para certificar suscriben en unidad de acto el/la Director/a – Rector/a ";
echo "con el/la Secretario/a General del Plantel o Profesor/a de Grado.</p>";
echo "<br><br><br><br><br><br><br><br>";
echo "<div style='width: 400px; float: left; margin-left: 50px;'>f) DIRECTOR/A o RECTOR/A</div><div style='width: 150px; float: right; margin-right: 50px;'>f) SECRETARIO/A</div>";
...
echo "<span style='font-size:8px;'><center>EBJA-$codigo</center></span>";
echo "<p>$distrito</p><center>";
echo "<p style='margin-top: 0;margin-bottom: 0;font-size: 24px; font-weight: bold;'>CERTIFICADO DE CALIFICACIONES</p>";
echo "<p style='margin-top: 0;margin-bottom: 0;font-size: 14px;'>PERIODO EDUCATIVO: $oferta->periodo</p></center>";
echo "<center><p style='font-size: 18px; font-weight: bold;'>$ie_nombre</p></center>";
echo "<p style='text-align: justify;margin-top: 0;margin-bottom: 0'>De conformidad con lo prescrito en el Art. 197 del Reglamento General a la Ley Orgánica ";
echo "de Educación Intercultural y demás normas vigentes, certifica que el/la estudiante:</p><br>";
echo "<p style='margin-top: 0;margin-bottom: 0;text-align: center;font-size: 18px; font-weight: bold; font-style: italic;'>$matriculado->nombres_aspirantes</p><br>";
echo "<p style='text-align: justify;margin-top: 0;'>Obtuvo las siguientes calificaciones durante el TERCER CURSO DE BACHILLERATO GENERAL UNIFICADO.</p>";
echo "<center>";
...
//iterar calificaciones
$total_materias = 1;
if(isset($calificaciones_fase_3)){	
	if(count($calificaciones_fase_3)>0){
		$total_materias = count($calificaciones_fase_3);
	}
	$promedio_final = 0;
	foreach ($calificaciones_fase_3 as $i => $cal) {
		$promedio_final += $cal->nota_final;
		echo "<tr>";
		echo "<td style='border: black 1px solid;'>".strtoupper($cal->asignatura)."</td>";		
		echo "<td style='border: black 1px solid;text-align:center;'>".escribirNota($cal->nota_final)."</td>";
...
echo "<tr style='background-color: #e6e6e6;'>";
echo "<td style='border: black 1px solid;font-weight: bold;'>EVALUACIÓN DEL COMPORTAMIENTO</td>";
if(isset($calificaciones_fase_3) and count($calificaciones_fase_3)>0){
	echo "<td colspan='2' style='border: black 1px solid;text-align:center;'>".recuperarComportamiento($calificaciones_fase_3[$total_materias-1]->comportamiento)."</td>";
}else{
	echo "<td colspan='2' style='border: black 1px solid;text-align:center;'></td>";
}
echo "</tr>";
