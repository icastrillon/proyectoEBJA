...
function desertar(codigo_inscripcion, elemento){
	...
	var q1p3 = document.getElementById('q1p3-'+codigo_inscripcion);
	...
	var q2p3 = document.getElementById('q2p3-'+codigo_inscripcion);
...
function calcular_pm_parciales_q1(codigo_inscripcion){
	...
	if(isNaN(q1p3.value)){
		q1p3.value = 0.00;
		return;
	}else if(q1p3.value * 1 > 10 || q1p3.value * 1 < 0){
		q1p3.value = 0.00;
		return;
	}

	var pm = (q1p1.value * 1 + q1p2.value * 1 + q1p3.value * 1)/3;
...
function calcular_pm_parciales_q2(codigo_inscripcion){
	...
	if(isNaN(q2p3.value)){
		q2p3.value = 0.00;
		return;
	}else if(q2p3.value * 1 > 10 || q2p3.value * 1 < 0){
		q2p3.value = 0.00;
		return;
	}
...
function guardar_basica(event){
	...
	if(contadorComportamiento > 0){
		...
		swal('Calificaciones Básica Superior Intensiva','Registro de calificaciones exitoso','success');
		setTimeout(document.getElementById('frm_guardar_basica').submit(),4000);
	}
}